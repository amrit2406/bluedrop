      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <motion.p
            ...
            viewport={{ once: true }}
          >
            We combine <span className="font-medium text-blue-500">advanced technology</span> with{' '}
            <span className="font-medium text-cyan-500">exceptional service</span> for the purest water experience.
          </motion.p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {items.map((item, index) => (
            ...
            >
              <div className={`absolute -inset-1 rounded-xl ${item.bg} opacity-0 group-hover:opacity-100 blur-md transition-all duration-300`}></div>
              
              <motion.div
                className={`relative p-8 rounded-xl bg-white shadow-md hover:shadow-lg transition-all duration-300 h-full flex flex-col items-center ${item.bg} bg-opacity-20 backdrop-blur-sm border border-white border-opacity-30`}
                whileHover={{ y: -5 }}
